...
def get_hash(arr):
    ...
    return (sexo, age, segmento, ind_empleado, ind_actividad_cliente, indresi)
...
def run_solution():
    ...
    sub_file = os.path.join('../data/submissions/zfturbo_submission_' + str(map7) + '_' + str(datetime.datetime.now().strftime("%Y-%m-%d-%H-%M")) + '.csv')
    out = open(sub_file, "w")
    f = open("../data/test_ver2.csv", "r")
    f.readline()
    total = 0
    count_empty = 0
